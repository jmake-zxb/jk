package com.jk.common.security.service;
 ...
 * @author 1661
 */
public interface JkUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new JkUser(info.getUserId(), info.getDept().getDeptId(), info.getUsername(),
	 ...
	 * @return 用户详情信息
	 */
	default UserDetails loadUserByUser(JkUser jkUser) {
		return this.loadUserByUsername(jkUser.getUsername());
	}
